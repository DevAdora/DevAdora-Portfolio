...
function Modal({
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 w-full max-w-4xl max-h-[85vh] bg-[#000000] border border-[#f0ede4]/10 rounded-2xl flex flex-col shadow-2xl">
        <div className="flex items-center justify-between px-6 py-5 border-b border-[#f0ede4]/10 shrink-0">
          <h2 className="text-sm font-semibold tracking-[0.2em] uppercase text-[#f0ede4]">
            {title}
          </h2>
          <button
            onClick={onClose}
            className="text-[#f0ede4]/50 hover:text-[#f0ede4] transition-colors p-1"
...
export default function AboutPage() {
  ...
  const achievementsRaw = [
    {
      title: "Latin Honors",
      year: "2025",
      description: "Graduated with Cum Laude in Southland College, and achieved Top 3 Overall academic rankings.",
  ...
  const experienceRaw = [
    ...
    {
      position: "Freelance Framer Developer",
      company: "Nechama",
      timeframe: "Jan 2025 - Present",
    },
    {
      position: "Freelance Desktop Application Developer",
      company: "Cognito",
  ...
  const experience = [...experienceRaw].sort(
    (a, b) => parseYear(b.timeframe) - parseYear(a.timeframe),
  );
  ...
  const AchievementCard = ({ item }: { item: (typeof achievements)[0] }) => (
    <div className="flex flex-col gap-1 p-4 border border-[#f0ede4]/10 rounded-xl hover:border-[#f0ede4]/25 transition-colors">
      <div className="flex items-start justify-between gap-2">
        <h3 className="text-sm font-semibold leading-snug text-[#f0ede4]">
          {item.title}
        </h3>
        <span className="text-[10px] text-[#f0ede4]/40 tracking-widest shrink-0 mt-0.5 uppercase">
          {item.year}
        </span>
      </div>
      <span className="text-[11px] font-medium text-[#f0ede4]/50 tracking-wide uppercase">
        {item.place}
      </span>
      <p className="text-xs text-[#f0ede4]/60 leading-relaxed mt-1">
        {item.description}
      </p>
    </div>
  );
  ...
  const ExperienceCard = ({ item }: { item: (typeof experience)[0] }) => (
    <div className="flex flex-col gap-1 p-4 border border-[#f0ede4]/10 rounded-xl hover:border-[#f0ede4]/25 transition-colors">
      <div className="flex items-start justify-between gap-2">
        ...
        <span className="text-[10px] text-[#f0ede4]/40 tracking-wide shrink-0 mt-0.5 whitespace-nowrap">
          {item.timeframe}
        </span>
      </div>
      <p className="text-xs text-[#f0ede4]/60 leading-relaxed">
        {item.company}
      </p>
    </div>
  );
  ...
  const ViewAllButton = ({
    ...
    <button
      onClick={onClick}
      className="w-full mt-4 py-3 rounded-xl border border-[#f0ede4]/15 text-[#f0ede4]/60 text-xs tracking-widest uppercase hover:border-[#f0ede4]/35 hover:text-[#f0ede4] transition-all duration-200 flex items-center justify-center gap-2"
    >
  ...
  return isLoading ? (
    ...
    <>
      ...
      <section className="min-h-screen px-[5%] py-12 bg-[#000000] text-[#f0ede4]">
        <div className="flex flex-col md:flex-row gap-8">
          <div className="md:w-1/3 md:sticky top-20 self-start">
            ...
            />
            <div className="text-[1.2rem] sm:text-[1.5rem] md:text-[1.6rem] lg:text-[1.8rem] xl:text-[2rem] leading-7 sm:leading-10 text-center md:text-left mt-4">
              <h1>DevAdora (Rai M. Reyes Jr.)</h1>
            </div>
            <div className="text-[1rem] sm:text-[1.1rem] md:text-[1.2rem] lg:text-[1.3rem] xl:text-[1.4rem] leading-5 sm:leading-7 text-center md:text-left mt-4">
              <h1>
          ...
          <div className="md:w-2/3">
              ...
              <div className="relative border-l-2 border-gray-400">
                {education.map((item, index) => (
                  <div key={index} className="mb-10 ml-6 relative">
                    <div className="absolute -left-[34px] w-6 h-6 bg-[#f0ede4] rounded-full border-2" />
                    <h3 className="text-lg font-semibold">{item.level}</h3>
                    <p className="text-sm text-[#f0ede4]/70">{item.school}</p>
                    <p className="text-sm text-[#f0ede4]/50">{item.years}</p>
                    {item.description && (
                      <p className="text-sm text-[#f0ede4]/60 mt-1">
                        {item.description}
                      </p>
                    )}
                  </div>
            ...
            <div className="p-0 sm:p-0 md:p-8 w-full">
              <div className="flex items-center justify-between mb-6">
                ...
                <span className="text-xs text-[#f0ede4]/40 tracking-widest uppercase">
      ...
      <Modal
        ...
        title={`All Work Experience (${experience.length})`}
      >
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
